...
void blk_SHA256_CTX::update(std::string_view str) {
    ...
    if (len_buf) {
        ...
        if (len_buf)
            return;
        blk_SHA256_Transform(this, buf);
    }
    while (len >= 64) {
        blk_SHA256_Transform(this, static_cast<const unsigned char *>(data));
...
std::array<unsigned char, 32> blk_SHA256_CTX::final() {
    // void blk_SHA256_Final(unsigned char *digest, blk_SHA256_CTX *ctx) {
    auto ret = std::array<unsigned char, 32>{};

    unsigned char *digest = ret.data();
    ...
    {
        size_t l = 1 + (63 & (55 - i));
        update(std::string_view{reinterpret_cast<const char *>(pad), l});
    }
    update(std::string_view{reinterpret_cast<const char *>(padlen), 8});

    /* copy output */
    for (i = 0; i < 8; i++, digest += sizeof(uint32_t))
        put_be32(digest, state[i]);
